...
4. 루틴 시작과 완료를 기록하고 도움됨 점수를 남깁니다.
5. 검증 대시보드에서 퍼널, KPI, 세그먼트별 결과를 확인합니다.

## 사용자 검증 KPI

화면의 결과는 측정 구조를 설명하기 위해 생성한 합성 시뮬레이션 데이터입니다. 실제 사용자 조사 결과가 아닙니다.
...
- `public/data/synthetic_validation_sessions.csv`: 가상 참여자 40명, 추천 노출 104세션
- `public/data/synthetic_event_log.csv`: 랜딩부터 피드백까지 544개 이벤트
- `public/data/synthetic_kpi_summary.csv`: 분자·분모·목표값을 포함한 KPI 9개
...
- TypeScript
- 순수 CSS 기반 반응형 UI
- CSV 기반 검증 데이터와 재현 가능한 생성 스크립트

## 실제 검증으로 전환할 때

현재 화면의 완료·평가 동작은 프로토타입 인터랙션입니다. 실제 사용자 데이터를 수집하려면 `landing_view`, `routine_builder_start`, `recommendation_view`, `routine_start`, `routine_complete`, `feedback_submit` 이벤트를 분석 도구나 데이터베이스로 전송하고 개인정보 고지와 동의를 추가해야 합니다.
